...
export default function CongressDashboard() {
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file || !congress || !congressId) return
  ...
  async function handleUpdateUser(e: React.FormEvent) {
    e.preventDefault()
    if (!editingUser || !congressId) return
  ...
  async function handleDeleteUser() {
    if (!deletingUser || !congressId) return
...
if (!congress || !congressId) {
  console.log('❌ No congress:', { congress, congressId }) // ← AGREGA ESTO
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center">
      ...
      <div className="px-4 py-6">
        ...
        {activeTab === 'mapa' && (
          <MapViewer 
            ...
            canEdit={true}
            onMapUpdated={() => loadCongressData(congressId)}
          />
        )}
